...
const TableServerList = ({ serverList, loading, handleFetch }) => {
  ...
  const handleFilter = (e) => {
    ...
    if(pickedFilter === "filterByDistance") {
      ...
      sortByDistance.asc ? filteredList = sort(filteredList, "distance", "asc") : filteredList = sort(filteredList, "distance", "desc");;
    }
    ...
    if(pickedFilter === "filterByName") {
      ...
      sortByName.asc ? filteredList = sort(filteredList, "name", "asc") : filteredList = sort(filteredList, "name", "desc");;
    }
  ...
  return (
    <table className="server-list-table">
        ...
        <tr>
          <th>
            <button onClick={handleFilter} name="filterByName">
              ...
              <img 
                src={Triangle} 
                style={{transform: `${sortByName.asc ? "rotate(180deg)" : "rotate(0deg)"}`, opacity: `${sortByName.isActive ? 1 : 0.5}`}} 
                alt="sorting icon"/>
            </button>
          </th>
          <th>
            <button onClick={handleFilter} name="filterByDistance">
              ...
              <img 
                src={Triangle} 
                style={{transform: `${sortByDistance.asc ? "rotate(180deg)" : "rotate(0deg)"}`, opacity: `${sortByDistance.isActive ? 1 : 0.5}`}} 
                alt="sorting icon"/>
            </button>
          </th>
        </tr>
      ...
      <tbody className="server-list-table__body">
        {loading ? <tr><td>Loading</td></tr> : filteredList.map(server => <Tr key={server.name + server.distance} server={server}/>)}
      </tbody>
    </table>
  );
};
